 ...
 *
 * Source of truth: VaultInstances table (actual vault state)
 * NATS accounts are just messaging infrastructure and should not determine enrollment status.
 *
 * Statuses:
 * - not_enrolled: No vault instance exists
 * - pending: Enrollment session in progress, waiting for mobile app
 * - enrolled: Vault instance exists and is ready for use
...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
  ...
  try {
    ...
    // No vault instance, no pending session
    return ok({
      status: 'not_enrolled',
    }, origin);
