...
/**
 * List credential recovery requests
 *
 * Query params:
 * - status: Filter by status (pending, approved, cancelled, completed, expired) - default all
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  try {
    ...
    if (statusFilter) {
      // Use scan with filter (could use GSI if available)
    ...
    const formattedRequests = requests.map(r => {
      ...
      return {
        ...
        status: r.status,
        recovery_type: r.recovery_type || 'lost_device',
    ...
    console.error('Error listing credential recovery requests:', error);

    await putAudit({
      type: 'admin_list_credential_recovery_error',
      error: error instanceof Error ? error.message : String(error)
    });

    return internalError('Failed to list credential recovery requests');
  }
};
