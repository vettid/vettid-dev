...
export class NitroStack extends cdk.Stack {
  ...
  private createUserData(): ec2.UserData {
    ...
    userData.addCommands(
      ...
      '# Resource allocation optimized for native Go handlers (no WASM overhead)',
      '# c6a.2xlarge: 8 vCPUs, 16 GB RAM - allocating 12 GB / 6 vCPUs to enclave (max)',
      '',
      'enclave:',
      ...
      'sleep 3',
      '',
      '# Restart (not start) the parent service — vettid-parent.service',
      '# is enabled-on-boot in the AMI, so systemd starts it BEFORE',
      '# user-data finishes writing /etc/vettid/parent.yaml above. The',
      '# initial process loads a partial yaml (KMS / DDB / log level',
      '# fields empty), the warnings fire silently, and users land on',
      '# a broken instance until something restarts the process. By',
      '# always calling `restart` here, we force a yaml re-read after',
      '# the full file is in place. Observed live: deploy 2026-05-25-v8',
      '# locked out both test users with the partial-config parent.',
      'echo "Restarting parent service (after writing parent.yaml)..."',
      'systemctl restart vettid-parent || true',
      '',
      '# Check status',
      'echo "=== Service Status ==="',
