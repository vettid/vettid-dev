...
export class NatsStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: NatsStackProps) {
    ...
    // SECURITY: Generates a random initialization token - actual nkey must be generated
    // by the nsc tool and stored manually. The nats-server will fail to start until
    // proper operator JWT is stored in this secret.
    // Initialize with: nsc generate nkey --operator && nsc push -a
    this.operatorSecret = new secretsmanager.Secret(this, 'NatsOperatorSecret', {
      secretName: 'vettid/nats/operator-key',
      description: 'NATS operator signing key for JWT-based authentication - REQUIRES NSC INITIALIZATION',
      generateSecretString: {
        secretStringTemplate: JSON.stringify({
          status: 'UNINITIALIZED',
          instructions: 'Initialize with: nsc generate nkey --operator && nsc add operator VettID && nsc push -a',
        }),
    ...
    // SECURITY: This secret is auto-populated by the first NATS node on boot
    // via the user data script (generates CA, stores in Secrets Manager)
    this.internalCaSecret = new secretsmanager.Secret(this, 'NatsInternalCaSecret', {
      secretName: 'vettid/nats/internal-ca',
      description: 'Internal CA for NATS cluster TLS - auto-generated by first node',
      generateSecretString: {
        secretStringTemplate: JSON.stringify({
          status: 'PENDING_FIRST_NODE',
          description: 'Will be auto-populated when first NATS node starts',
        }),
        generateStringKey: 'initialization_token',
        excludePunctuation: true,
        passwordLength: 32,
