...
const (
	...
	// TURN credentials (parent fetches Cloudflare TURN secret + generates HMAC creds)
...
// GetTurnCredentials returns short-lived Cloudflare TURN credentials for the
// vault's owning user. Generated by the parent (Secrets Manager + HMAC).
// Result is the raw JSON body to pass straight back to the app.
func (p *SealerProxy) GetTurnCredentials() ([]byte, error) {
	req := SealerRequest{
		Operation:  SealerOpGetTurnCredentials,
		OwnerSpace: p.ownerSpace,
	}
	resp, err := p.sendRequest(req)
	if err != nil {
	...
	if !resp.Success {
		return nil, fmt.Errorf("get turn credentials error: %s", resp.Error)
	}
	if len(resp.TurnCredentials) == 0 {
		return nil, fmt.Errorf("empty turn credentials response")
	}
	return resp.TurnCredentials, nil
}
