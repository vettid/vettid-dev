...
// EncryptedStorage provides encrypted key-value storage for org vaults.
// In production, this uses S3-backed encrypted SQLite (same as user vaults).
...
// Delete removes a key and securely zeros the old value
func (s *EncryptedStorage) Delete(key string) error {
	s.mu.Lock()
	defer s.mu.Unlock()

	if old, exists := s.data[key]; exists {
		// SECURITY: Zero the old value before removing
		for i := range old {
			old[i] = 0
		}
	}
...
// SecureErase zeros all data in storage
func (s *EncryptedStorage) SecureErase() {
	s.mu.Lock()
	defer s.mu.Unlock()
