...
- **#240** — **Decommission flow does not delete S3 vault objects under `vaults/{user_guid}/`.** Surfaced 2026-05-16 during the test pass: mesmer (Pixel 7, user_guid `af44310d`) had a stale `vault_state.enc` from a prior session that survived decommission. Combined with D3's fresh-enrollment IfNoneMatch:* write, this would have bricked enrollment if the D3-first-write fix (vettid-dev `c5b1317`) hadn't shipped. The decommission Lambda needs to `aws s3 rm --recursive s3://<vault-bucket>/vaults/<user_guid>/` so re-enrollment starts from a truly clean slate (vault_state.enc, sealed_material.bin, sealed_ecies.bin all gone). Today operators have to manually `aws s3 rm` between decommission and re-enrollment.
