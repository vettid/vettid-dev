...
- ~~**#240** — Decommission flow does not delete S3 vault objects under `vaults/{user_guid}/`.~~ ✅ vettid-dev `237dcee` + CDK deploy (reorder: enclave reset → 3s wait → S3 sweep, catches any final auto-persist by the dying vault-manager).
- ~~**#239** — OLD enclave accepts fresh enrollments during migration window.~~ ✅ vettid-dev `c19aa44` (RoutingManager.SetCurrentPCR0Reader; ClaimForEnrollment silently declines when local PCR0 != SSM current. Ships in next enclave deploy.)
